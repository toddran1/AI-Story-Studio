...
import type { VisualPreflightReport } from "./VisualProfileCheckDialog.js";
...
export function SummaryLayers({ slug, summary, busy, children, onChange, onGenerate, onError }: Props) {
  ...
  const [produceBlocked, setProduceBlocked] = useState<{ id: string; preflight: VisualPreflightReport }>();
  useEffect(() => { setText(summary.narration?.text ?? ""); }, [summary.id, summary.narration?.text]);
  useEffect(() => {
    setProduceBlocked(undefined);
    const receive = (event: Event) => {
      const result = (event as CustomEvent<{ id?: string; reason?: string; preflight?: VisualPreflightReport }>).detail;
      if (result?.id === summary.id && result.reason === "visual-profile-decisions-required" && result.preflight) {
        setProduceBlocked({ id: result.id, preflight: result.preflight }); setTab("video");
      }
    };
    window.addEventListener("summary-produce-blocked", receive);
    return () => window.removeEventListener("summary-produce-blocked", receive);
  }, [summary.id]);
  ...
  return <div className="summary-layers"><nav className="summary-layer-tabs" aria-label="Summary layers">
    ...
    </div> : tab === "scenes" ? null : tab === "artwork" ? <SummaryArtworkPanel key={summary.id} slug={slug} summary={summary} base={base} disabled={disabled} onChange={onChange} onGenerate={onGenerate} onError={onError} onEditScene={(sceneId) => { setFocusSceneId(sceneId); setTab("scenes"); }} /> : tab === "video" ? <SummaryVideoPanel key={summary.id} slug={slug} summary={summary} base={base} disabled={disabled} onChange={onChange} onGenerate={onGenerate} onError={onError} produceBlocked={produceBlocked} /> : <div className="summary-media-editor">

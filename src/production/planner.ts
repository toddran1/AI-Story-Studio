...
export async function buildProductionPlan(options: { root: string; story: Story; chapters: number[]; outputs: ProductionOutput[]; artwork: boolean; force?: ProductionForce }): Promise<ProductionPlan> {
  ...
  let imageOperations = 0; let imagesPendingPlanning = 0;
  for (const number of options.chapters) {
    ...
    if (stages.includes("artwork")) {
      ...
      if (!manifest?.success) imagesPendingPlanning++;
  ...
  return { story: options.story.slug, from: options.chapters[0]!, to: options.chapters.at(-1)!, chapters: options.chapters, outputs: options.outputs, artwork: options.artwork, stages, counts,

...
export function BiblePage({ slug, navigate, locationSearch }: { slug: string; navigate: (path: string, options?: NavigateOptions) => void; locationSearch: string }) {
  ...
  const [mergeReview, setMergeReview] = useState<{ target: any; source: any; reason: string; kind?: "narration_rendering_duplicate"; recommendedTargetEntityId?: string } | null>(null);
  ...
  const [impactPreview, setImpactPreview] = useState<{ title: string; diff: Array<{ label: string; before: string; after: string }>; impact: EntityImpact; applyLabel?: string; reasonRequired?: boolean; apply: (reason?: string) => Promise<void> } | null>(null);
  ...
  const requestImpact = async (entityId: string, body: Record<string, unknown>, preview: { title: string; diff?: Array<{ label: string; before: string; after: string }>; applyLabel?: string; reasonRequired?: boolean; apply: (reason?: string) => Promise<void> }) => {
    try { setError(""); setImpactError(""); setImpactReason(""); const impact = await post<EntityImpact>(`/stories/${slug}/story-bible/entities/${entityId}/impact`, body); setImpactPreview({ title: preview.title, diff: preview.diff ?? [], impact, applyLabel: preview.applyLabel, reasonRequired: preview.reasonRequired, apply: preview.apply }); }
    catch (value) { setError(message(value)); }
  };
  const runImpactApply = async () => { if (!impactPreview || (impactPreview.reasonRequired && !impactReason.trim())) return; try { setImpactBusy(true); setError(""); setImpactError(""); await impactPreview.apply(impactReason.trim()); setImpactPreview(null); } catch (value) { setImpactError(message(value)); } finally { setImpactBusy(false); } };
  ...
  const merge = async (item: any) => { try { const [a, b] = await Promise.all(item.entities.map((entity: any) => api<any>(`/stories/${slug}/story-bible/entities/${entity.id}`))); setMergeReview({ target: item.recommendedTargetEntityId === b.entity.id ? b : a, source: item.recommendedTargetEntityId === b.entity.id ? a : b, reason: item.reason, kind: item.kind, recommendedTargetEntityId: item.recommendedTargetEntityId }); } catch (value) { setError(message(value)); } };
  ...
  const confirmMerge = async () => { if (!mergeReview) return; const target = mergeReview.target.entity; const source = mergeReview.source.entity; const openEntityId = requestedEntityId; const kind = mergeReview.kind === "narration_rendering_duplicate" && mergeReview.target.entity.id === mergeReview.recommendedTargetEntityId ? mergeReview.kind : "standard"; await requestImpact(source.id, { action: "merge", targetEntityId: target.id }, { title: `Merge ${source.canonicalName} into ${target.canonicalName}`, diff: [{ label: "Merge", before: `${source.canonicalName} (${source.id})`, after: `Merged into ${target.canonicalName}` }], applyLabel: "Confirm merge", apply: async () => { await post(`/stories/${slug}/story-bible/merges`, { targetEntityId: target.id, sourceEntityIds: [source.id], reason: `Approved duplicate suggestion: ${mergeReview.reason}`, kind }); setMergeReview(null); if (openEntityId === source.id) closeEntitySheet(source.id); else if (openEntityId === target.id && parseBibleQuery(location.search).entity === target.id) await loadEntityDetail(target.id, true); await load(); } }); };
  const suppress = async (record: any) => {
    const entity = record.entity;
    await requestImpact(entity.id, { action: "suppress" }, {
      title: `Remove ${entity.canonicalName}`,
      diff: [{ label: "Suppression", before: "Canonical entity", after: "Removed from the effective Story Bible (restorable)" }],
      applyLabel: "Remove entity", reasonRequired: true,
      apply: async (reason) => {
        await post(`/stories/${slug}/story-bible/entities/${entity.id}/suppress`, { reason });
        if (requestedEntityIdRef.current ? requestedEntityIdRef.current === entity.id : detailEntityIdRef.current === entity.id) {
          closeEntitySheet(entity.id);
          navigate(`/stories/${slug}/bible${bibleQueryString({ tab, type, q: debouncedQuery, sort, readiness, page })}`, { scroll: "preserve", replace: true });
        }
        setNotice(`Removed "${entity.canonicalName}" from the effective Story Bible. You can restore it below.`);
        await load();
      },
    });
  };
  ...
  return <section className="page canonical-page"><div className="section-heading"><div><span className="eyebrow">Long-form memory</span><h2>Story Bible</h2><p>Canonical identities, aliases, history, relationships, and traceable evidence.</p></div><div className="production-head-actions"><button className="button" onClick={() => navigate(`/stories/${slug}/names`)}>Names / Localization</button><button className="button" onClick={() => navigate(`/stories/${slug}/continuity`)}>Continuity review</button></div></div>{error && <ErrorBox text={error} />}{notice && <div className="naming-notice">{notice}</div>}{health ? <StoryBibleHealthCard health={health} onReviewAll={() => setTab("review")} onOpenCleanup={() => setTab("cleanup")} /> : healthError ? <section className="bible-health" aria-label="Story Bible Health"><div className="bible-health-head"><div><span className="eyebrow">Story Bible Health</span><b>Health summary unavailable</b></div><button className="button" onClick={() => void loadHealth()}>Retry</button></div></section> : null}
        ...
        {analysis.recommendations?.map((rec: any) => <article key={rec.id} className="continuity-card" style={{ padding: "16px", borderRadius: "8px", background: "#141419", border: "1px solid var(--line)" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "16px" }}>
            ...
            <div style={{ display: "flex", gap: "8px", flexShrink: 0 }}>
              ...
              {rec.recommendation === "needs_review" && rec.targetEntityId && <button className="button" style={{ fontSize: "10px" }} onClick={() => merge({ entities: [{ id: rec.targetEntityId }, { id: rec.entityId }], recommendedTargetEntityId: rec.targetEntityId, reason: rec.reason, kind: rec.kind })}>Review & merge</button>}
            </div>
          </div>
        </article>)}
    ...
    {impactPreview && <EntityImpactDialog title={impactPreview.title} diff={impactPreview.diff} impact={impactPreview.impact} busy={impactBusy} applyLabel={impactPreview.applyLabel} reasonRequired={impactPreview.reasonRequired} reason={impactReason} onReasonChange={setImpactReason} error={impactError} onCancel={() => setImpactPreview(null)} onApply={runImpactApply} />}
...
export function EntityImpactDialog({ title, diff = [], impact, busy = false, applyLabel = "Apply change", reasonRequired = false, reason = "", onReasonChange, error, onCancel, onApply }: { title: string; diff?: Array<{ label: string; before: string; after: string }>; impact: EntityImpact; busy?: boolean; applyLabel?: string; reasonRequired?: boolean; reason?: string; onReasonChange?: (value: string) => void; error?: string; onCancel: () => void; onApply: () => void }) {
  ...
  return <div className="editor-sheet naming-editor entity-impact-dialog" role="dialog" aria-modal="true" aria-label="Review estimated impact">
    ...
    {reasonRequired && <><p>This removes the separate canonical record from current Story Bible views. Its historical evidence remains available and the record can be restored. Aliases on other entities are preserved.</p><label>Reason for removal <input aria-label="Reason for removal" value={reason} maxLength={1000} onChange={(event) => onReasonChange?.(event.target.value)} placeholder="For example: duplicate alias of Hades" /></label></>}
    {error && <div role="alert" className="error-box">{error}</div>}
    <div className="editor-sheet-actions"><button className="button" onClick={onCancel}>Cancel</button><button className="button primary" disabled={busy || (reasonRequired && !reason.trim())} onClick={onApply}>{busy ? "Removing…" : applyLabel}</button></div>
  </div>;
}
    ...
    try {
      const response = await put<any>(`/stories/${slug}/settings`, {
        ...
        tts: {
          ...
          qualityMode: story.pipeline.tts.qualityMode ?? "off",
  ...
  return <section className="page settings-page">
    ...
    <form onSubmit={save}>
      ...
      <div className="settings-group"><h3>Voice</h3>
        ...
        <Field label="Post-generation audio quality mode"><select value={story.pipeline.tts.qualityMode ?? "off"} onChange={(event) => setStory({ ...story, pipeline: { ...story.pipeline, tts: { ...story.pipeline.tts, qualityMode: event.target.value as "off" | "verify" | "auto_repair" } } })}><option value="off">Off — generate once</option><option value="verify">Verify only — transcribe/check, no Fish retries</option><option value="auto_repair">Auto repair — transcribe/check and retry failed chunks</option></select><small className="field-note">Off is the default. Verification and auto-repair run only when explicitly selected.</small></Field>

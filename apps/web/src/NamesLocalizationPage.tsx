...
export function NamesLocalizationPage({ slug, onJob, navigate }: { slug: string; onJob: (job: Job) => void; navigate: (path: string) => void }) {
  ...
  const [selected, setSelected] = useState<string | undefined>(requested ?? undefined); const [query, setQuery] = useState(""); const [debouncedQuery, setDebouncedQuery] = useState("");
  ...
  const entityRequest = useRef(0); const listRequest = useRef(0); const suggestionRequest = useRef(0); const listAbort = useRef<AbortController>();
  const loadEntities = async () => { listAbort.current?.abort(); const controller = new AbortController(); listAbort.current = controller; const request = ++listRequest.current; try { const value = await api<any>(`/stories/${slug}/story-bible/entities?page=${page}&pageSize=50&type=${type}&sort=name&q=${encodeURIComponent(debouncedQuery)}`, { signal: controller.signal }); if (request === listRequest.current) { setView(value); setListError(""); } return value; } catch (value) { if (request === listRequest.current && !controller.signal.aborted) setListError(message(value)); return undefined; } };
  ...
  useEffect(() => { const timer = window.setTimeout(() => setDebouncedQuery(query.trim()), 300); return () => clearTimeout(timer); }, [query]);
  useEffect(() => { if (debouncedQuery !== query.trim()) { listAbort.current?.abort(); listRequest.current++; return; } void loadEntities(); return () => { listAbort.current?.abort(); listRequest.current++; }; }, [slug, page, type, debouncedQuery, query]);
  useEffect(() => { const request = ++entityRequest.current; setDetail(undefined); setDraft(undefined); if (!selected) return; api<any>(`/stories/${slug}/story-bible/entities/${selected}`).then((value) => {
    if (request !== entityRequest.current || value.entity.id !== selected) return; setDetail(value); const naming = value.entity.localizedNaming; setDraft({ locale: naming?.locale ?? defaultLocale(story?.outputLanguage), fullName: naming?.fullName ?? value.entity.preferredNarrationName ?? "", shortName: naming?.shortName ?? "", usageMode: naming?.usageMode ?? "ai_contextual", notes: naming?.notes ?? "" });
  }).catch((value) => { if (request === entityRequest.current) setDetailError(message(value)); }); }, [slug, selected, story?.outputLanguage]);
  useEffect(() => () => { entityRequest.current++; listRequest.current++; listAbort.current?.abort(); suggestionRequest.current++; if (pollTimer.current !== undefined) clearTimeout(pollTimer.current); }, []);

  const choose = (id: string) => { entityRequest.current++; suggestionRequest.current++; if (pollTimer.current !== undefined) clearTimeout(pollTimer.current); setGenerating(false); setSelected(id); setDetail(undefined); setDraft(undefined); setSuggestions([]); setNotice(""); history.replaceState({}, "", `/stories/${slug}/names?entity=${id}`); };
  const save = async () => { if (!draft || !selected || saving) return; const entityId = selected; const request = entityRequest.current; try { setSaving(true); setError(""); const response = await put<any>(`/stories/${slug}/story-bible/entities/${entityId}`, { localizedNaming: { locale: draft.locale.trim(), fullName: draft.fullName.trim() || undefined, shortName: draft.shortName.trim() || undefined, usageMode: draft.usageMode, notes: draft.notes.trim() || undefined } }); const affected = response.invalidation?.affectedChapters?.length ?? 0; const manual = response.invalidation?.manualNarrationChapters?.length ?? 0; if (request !== entityRequest.current) return; setNotice(affected ? `Saved. ${affected} affected chapter${affected === 1 ? "" : "s"} marked for regeneration.${manual ? ` ${manual} manual narration edit${manual === 1 ? " was" : "s were"} preserved for review.` : ""}` : "Localization saved."); setDetail((current: any) => current ? { ...current, entity: response.entity } : current); setView((current: any) => current ? { ...current, items: current.items.map((item: Entity) => item.id === entityId ? response.entity : item) } : current); void loadEntities(); } catch (value) { if (request === entityRequest.current) setError(message(value)); } finally { setSaving(false); } };
  const remove = async () => { if (!selected || !confirm("Remove this localized naming rule? Canonical and original names will remain unchanged.")) return; const entityId = selected; const request = entityRequest.current; try { setSaving(true); const response = await put<any>(`/stories/${slug}/story-bible/entities/${entityId}`, { localizedNaming: null }); if (request !== entityRequest.current) return; setDetail((current: any) => current ? { ...current, entity: response.entity } : current); setView((current: any) => current ? { ...current, items: current.items.map((item: Entity) => item.id === entityId ? response.entity : item) } : current); setDraft({ locale: defaultLocale(story?.outputLanguage), fullName: "", shortName: "", usageMode: "ai_contextual", notes: "" }); setSuggestions([]); setNotice("Localization removed. Affected generated outputs were marked stale."); void loadEntities(); } catch (value) { if (request === entityRequest.current) setError(message(value)); } finally { setSaving(false); } };
  const generate = async (count = 5) => { if (!selected || !draft || generating || detail?.entity?.id !== selected) return; const entityId = selected; const request = ++suggestionRequest.current; try { setGenerating(true); setError(""); const job = await post<Job>(`/stories/${slug}/story-bible/entities/${entityId}/localization-suggestions`, { locale: draft.locale.trim(), count }); if (request !== suggestionRequest.current) return; onJob(job); poll(job.id, entityId, request); } catch (value) { if (request === suggestionRequest.current) { setGenerating(false); setError(message(value)); } } };
  const poll = (id: string, entityId: string, request: number) => { const check = async () => { if (request !== suggestionRequest.current) return; try { const job = await api<Job>(`/jobs/${id}`); if (request !== suggestionRequest.current || selected !== entityId) return; onJob(job); if (job.status === "completed") { setSuggestions(job.result?.suggestions ?? []); setGenerating(false); return; } if (job.status === "failed" || job.status === "paused") { setError(job.error ?? "Name suggestions did not finish"); setGenerating(false); return; } pollTimer.current = window.setTimeout(check, 800); } catch (value) { if (request === suggestionRequest.current) { setError(message(value)); setGenerating(false); } } }; void check(); };

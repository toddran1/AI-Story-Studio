...
const productionInputSchema = z.object({ from: z.number().int().positive(), to: z.number().int().positive(), profile: z.string().optional(), outputs: z.array(productionOutputSchema).min(1).optional(), artwork: z.boolean().optional(), repairQa: z.boolean().optional(), alignment: z.boolean().optional(), refresh: z.boolean().default(false), dryRun: z.boolean().default(false), force: productionForceSchema.optional(), audiobookFormat: z.enum(["mp3", "m4b"]).optional() }).strict().refine((value) => value.to >= value.from, { message: "Range end must be at or after range start" });
...
export class StudioOperations {
  ...
  readonly queue?: ProductionQueueService;
  constructor(public readonly root: string, private readonly env: Environment, public readonly jobs = new JobManager(), dependencies: OperationsDependencies = {}) {
    ...
    this.scenePlanner = dependencies.scenePlanner; this.image = dependencies.image ?? new OpenAIImageProvider(env.OPENAI_API_KEY, env.PROVIDER_TIMEOUT_MS); this.tts = dependencies.tts ?? runtime.tts;
  ...
  async updateCanonicalEntity(slug: string, id: string, raw: unknown) { slugSchema.parse(slug); return withStoryLock(this.root, slug, "canonical entity edit", async () => { const base = await getStoryBible(this.root, slug, { includeCanonicalOverlay: false }); const result = await updateCanonicalEntity(this.root, slug, base, id, raw); await recordActivity(this.root, slug, "bible.entity.edited", `Updated canonical entity ${id}`); return { entity: result.bible.canonicalEntities.find((item) => item.id === id) }; }); }
  ...
  async resolveContinuity(slug: string, id: string, raw: unknown) { slugSchema.parse(slug); const input = z.object({ resolution: z.enum(["accepted_new", "kept_existing", "intentional", "corrected", "merged", "dismissed"]), note: z.string().trim().max(2000).optional() }).strict().parse(raw); return withStoryLock(this.root, slug, "continuity resolution", async () => { const finding = await resolveContinuityFinding(this.root, slug, id, input.resolution, input.note); await recordActivity(this.root, slug, "continuity.resolved", `Resolved ${finding.type} finding`); return { finding }; }); }

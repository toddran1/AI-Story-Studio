...
export function NamesLocalizationPage({ slug, onJob, navigate }: { slug: string; onJob: (job: Job) => void; navigate: (path: string) => void }) {
  ...
  const entityRequest = useRef(0); const listRequest = useRef(0); const suggestionRequest = useRef(0);
  const loadEntities = async () => { const request = ++listRequest.current; try { const value = await api<any>(`/stories/${slug}/story-bible/entities?page=${page}&pageSize=50&type=${type}&sort=name&q=${encodeURIComponent(deferred)}`); if (request === listRequest.current) setView(value); return value; } catch (value) { if (request === listRequest.current) throw value; return undefined; } };
  ...
  }).catch((value) => { if (request === entityRequest.current) setError(message(value)); }); }, [slug, selected, story?.outputLanguage]);
  useEffect(() => () => { entityRequest.current++; listRequest.current++; suggestionRequest.current++; if (pollTimer.current !== undefined) clearTimeout(pollTimer.current); }, []);
  ...
  const save = async () => { if (!draft || !selected || saving) return; const entityId = selected; const request = entityRequest.current; try { setSaving(true); setError(""); const response = await put<any>(`/stories/${slug}/story-bible/entities/${entityId}`, { localizedNaming: { locale: draft.locale.trim(), fullName: draft.fullName.trim() || undefined, shortName: draft.shortName.trim() || undefined, usageMode: draft.usageMode, notes: draft.notes.trim() || undefined } }); const affected = response.invalidation?.affectedChapters?.length ?? 0; const manual = response.invalidation?.manualNarrationChapters?.length ?? 0; await loadEntities(); if (request !== entityRequest.current) return; setNotice(affected ? `Saved. ${affected} affected chapter${affected === 1 ? "" : "s"} marked for regeneration.${manual ? ` ${manual} manual narration edit${manual === 1 ? " was" : "s were"} preserved for review.` : ""}` : "Localization saved."); setDetail({ ...detail, entity: response.entity }); } catch (value) { if (request === entityRequest.current) setError(message(value)); } finally { setSaving(false); } };
  const remove = async () => { if (!selected || !confirm("Remove this localized naming rule? Canonical and original names will remain unchanged.")) return; const entityId = selected; const request = entityRequest.current; try { setSaving(true); const response = await put<any>(`/stories/${slug}/story-bible/entities/${entityId}`, { localizedNaming: null }); await loadEntities(); if (request !== entityRequest.current) return; setDetail({ ...detail, entity: response.entity }); setDraft({ locale: defaultLocale(story?.outputLanguage), fullName: "", shortName: "", usageMode: "ai_contextual", notes: "" }); setSuggestions([]); setNotice("Localization removed. Affected generated outputs were marked stale."); } catch (value) { if (request === entityRequest.current) setError(message(value)); } finally { setSaving(false); } };

...
async function main() { const args = parse(process.argv.slice(2)); loadEnvironment(); await withStoryLock(process.cwd(), args.story, "chapter video rendering", async () => { let story = await loadStory(storyPaths(process.cwd(), args.story, 1).storyConfig); if (args.subtitleMode) story = { ...story, video: { ...story.video, subtitleMode: args.subtitleMode } }; const selected = selectChapterRange((await loadImportedChapters(process.cwd(), args.story)).chapters, args.from, args.to); const processor = new FfmpegVideoProcessor(); for (let index = 0; index < selected.length; index++) { const chapter = selected[index]!.chapter; if (story.video.subtitleMode !== "none") await generateStoredSubtitles({ root: process.cwd(), story, chapter }); const result = await renderStoredChapterVideo({ root: process.cwd(), story, chapter, processor, force: args.force }); process.stdout.write(`[${index + 1}/${selected.length}] Chapter ${chapter}: ${result.reused ? "reused" : "rendered"}\n`); } }); }

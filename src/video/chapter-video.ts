...
export async function renderStoredChapterVideo(options: { root: string; story: Story; chapter: number; processor: VideoProcessor; force?: boolean; onEvent?: (event: VideoEvent) => void }) {
  ...
  const subtitleArtifact = needsSubtitles ? await inspectStageArtifact(options.root, options.story.slug, options.chapter, "subtitles") : undefined;
  if (needsSubtitles && (subtitleArtifact!.availability !== "available" || !(await fileIsNonEmpty(paths.subtitlesSrt)))) throw new VideoError(`Chapter ${options.chapter} subtitles are not ready`);
  const warnings: string[] = [];
  if (audioArtifact.freshness === "stale") warnings.push(stalePrerequisiteWarning("audioMastering"));
  if (subtitleArtifact?.freshness === "stale") warnings.push(stalePrerequisiteWarning("subtitles"));
  const sceneArtwork = await approvedSceneArtwork(options.root, options.story.slug, options.chapter); const availableCover = await findCover(options.root, options.story.slug); const cover = sceneArtwork ? undefined : options.story.video.backgroundMode === "gradient" ? undefined : availableCover; const backgroundFingerprint = sceneArtwork ? fingerprint(sceneArtwork.map((item) => ({ durationSeconds: item.durationSeconds, fingerprint: item.fingerprint }))) : cover ? await fileFingerprint(cover) : "generated-fallback-v1"; const audioFingerprint = await fileFingerprint(paths.audio); const subtitleFileFingerprint = needsSubtitles ? await fileFingerprint(paths.subtitlesSrt) : undefined; const videoSettings = resolveVideoSettings(options.story.video); const inputFingerprint = videoFingerprint(audioFingerprint, subtitleFileFingerprint, backgroundFingerprint, videoSettings, chapter.translatedTitle ?? chapter.originalTitle);

...
export type QaResult = { status: "pass" | "warn" | "fail"; score: number; issues: Array<{ category: string; severity: "warn" | "fail"; message: string; evidence: string; review?: { disposition: "dismissed"; reviewedAt: string } }>; checks: Record<string, "pass" | "warn" | "fail"> };
...
export type StorySummary = { id:string;storyId:string;title:string;chapters:number[];chapterRange?:{from:number;to:number};summaryType:"brief"|"detailed"|"mini-chapter"|"arc"|"character-focused"|"custom";sourceMode:"original"|"translated"|"chapter-summaries";targetLength:{words:number};focus?:string;instructions?:string;text:string;status:"generating"|"complete"|"failed";origin:"generated"|"manual";manuallyEdited:boolean;contextEligible:boolean;error?:string;createdAt:string;updatedAt:string;provenance:{model:Model;promptVersion:string;chapterSources:Array<{chapter:number;mode:string;characters:number;fingerprint:string}>;levels:Array<{level:number;batches:Array<{batch:number;chapters:number[];inputCharacters:number}>}>} };

...
export const productionStageSchema = z.union([stageNameSchema, z.enum(["audiobook", "videoExport", "refresh", "budget"])]);
...
export const productionManifestSchema = z.object({
  ...
  selection: z.object({ from: z.number().int().positive(), to: z.number().int().positive() }), options: z.object({ outputs: z.array(productionOutputSchema).min(1), artwork: z.boolean(), repairQa: z.boolean(), refresh: z.boolean(), alignment: z.boolean().default(true), audiobookFormat: z.enum(["mp3", "m4b"]), force: productionForceSchema.optional(), profile: z.string().optional(), dryRun: z.boolean(), maxProviderBudgetUsd: z.number().positive().optional() }),

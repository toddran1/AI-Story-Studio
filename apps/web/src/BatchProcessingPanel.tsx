...
export function BatchProcessingPanel(props: Props) {
  ...
  const run = async () => {
    ...
    try {
      ...
      watcher.current = props.watchJob(job.id, (next) => {
        ...
        else if (next.status === "failed") { setRunning(false); setResult(next.result); setError(next.error ?? "Stage execution failed"); setPreview(undefined); setActiveJob(undefined); }
  ...
  const canRun = Boolean(preview && preview.summary.operationCount > 0 && preview.summary.blockedOperations === 0 && !running);
...
export function CompletionSummary({ result }: { result: any }) { const summary = result.summary ?? {}; const failures = (result.results ?? []).filter((item: { status: string }) => item.status === "failed"); return <div className="batch-completion" role="status"><b>{failures.length ? "Batch finished with errors" : "Batch complete"}</b><span>{summary.completedChapters ?? 0} chapters processed · {summary.completedOperations ?? 0} operations completed · {summary.reusedCount ?? 0} artifacts reused · {summary.blockedChapters ?? 0} blocked · {summary.failedChapters ?? 0} failed</span>{failures.length > 0 && <ul>{failures.map((item: { chapter: number; error?: string }) => <li key={item.chapter}>Chapter {item.chapter}: {item.error ?? "Stage execution failed"}</li>)}</ul>}</div>; }

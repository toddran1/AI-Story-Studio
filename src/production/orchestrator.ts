...
import { AlignmentConfig, AlignmentEngine } from "../alignment/types.js";
...
export async function planProduction(request: ProductionRequest, dependencies: Pick<ProductionDependencies, "loadChapters" | "refresh">): Promise<{ plan: ProductionPlan; chapters: DiscoveredChapter[]; resolved: ReturnType<typeof resolveProductionOptions> }> {
  ...
  const resolved = resolveProductionOptions(request.story, request); const plan = await buildProductionPlan({ root: request.root, story: request.story, chapters: chapters.map((item) => item.chapter), outputs: resolved.outputs, artwork: resolved.artwork, alignment: resolved.alignment, force: request.force }); if (request.refresh) { plan.stages.unshift("refresh"); plan.counts.refresh = { required: 1, reusable: 0 }; } return { plan, chapters, resolved };
}
...
export async function runProduction(request: ProductionRequest, dependencies: ProductionDependencies): Promise<{ manifest: ProductionManifest; plan: ProductionPlan }> {
  const started = Date.now(); const { plan, chapters, resolved } = await planProduction(request, dependencies); const manifestOptions: ProductionManifest["options"] = { outputs: resolved.outputs, artwork: resolved.artwork, repairQa: resolved.repairQa, refresh: Boolean(request.refresh), alignment: resolved.alignment, audiobookFormat: resolved.audiobookFormat, force: request.force, profile: request.profile, dryRun: Boolean(request.dryRun) };
...
function resumeSignature(options: ProductionManifest["options"]) { return { outputs: [...options.outputs].sort(), artwork: options.artwork, repairQa: options.repairQa, alignment: options.alignment, audiobookFormat: options.audiobookFormat, profile: options.profile ?? null }; }
...
function coreForce(force?: ProductionForce): PipelineOptions["force"] { if (!force) return undefined; if (["ingestion", "translation", "narration", "qa", "story-bible", "continuity", "tts", "audio", "all"].includes(force)) return force as PipelineOptions["force"]; return undefined; }
...
function fallbackAlignmentConfig(): AlignmentConfig { return { engine: "disabled", executable: "whisper-cli", device: "auto", minimumMatchPercentage: 85, minimumConfidence: .45, maximumGapSeconds: 15, timeoutMs: 1_800_000 }; }

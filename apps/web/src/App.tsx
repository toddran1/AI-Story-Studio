...
function StoryPage({ slug, navigate, onJob }: { slug: string; navigate: (path: string) => void; onJob: (job: Job) => void }) {
  ...
  const selectMatchingChapters = async (all = false) => { try { setError(""); const selected: number[] = []; let nextPage = 1; let pages = 1; do { const result = await api<{ items: ChapterRow[]; pages: number }>(`/stories/${slug}/chapters?page=${nextPage}&pageSize=100&filter=${all ? "all" : filter}&q=${all ? "" : encodeURIComponent(deferredQuery)}`); selected.push(...result.items.map((item) => item.chapter)); pages = result.pages; nextPage++; } while (nextPage <= pages); setSelectedChapters([...new Set(selected)].sort((left, right) => left - right)); } catch (value) { setError(message(value)); } };
...
export function QaDetail({ slug, chapter, onJob, onEditManually, onChanged, initialData, initialError }: { slug: string; chapter: number; onJob: (job: Job) => void; onEditManually: () => void; onChanged: () => void; initialData?: ChapterQaDetail; initialError?: string }) {
  ...
  return <div className="qa-detail stateful">
    ...
    {note && <div className="naming-notice">{note}{safeFixRecheckFailed && <button className="button" onClick={() => { setSafeFixRecheckFailed(false); void recheck("full"); }}>Retry QA</button>}</div>}

...
export async function masterStoredChapter(options: { root: string; story: Story; chapter: number; processor: AudioMasteringProcessor; force?: boolean; onEvent?: (event: AudioMasteringEvent) => void }) {
  ...
  const started = Date.now(); chapter.audio = undefined; chapter.stages.audioMastering = { status: "running", provider: "ffmpeg", model: options.processor.version, fingerprint: fp, startedAt: new Date().toISOString() };

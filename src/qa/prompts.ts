...
export const QA_PROMPT_VERSION = "9";
...
AUTHORITATIVE NARRATION-NAME RULE SEVERITY: When the narration violates an explicit naming rule in AUTHORIZED NARRATION NAMING MAPPINGS, report it as a names-category FAIL because it is a confirmed identity-rule violation, not a subjective style preference. This includes a Preferred Narration Name, a custom alias replacement, a use_preferred alias rule, and deterministic localizedNaming modes always_full or always_short. Check each occurrence independently: an authorized form appearing elsewhere in the chapter does not excuse a separate occurrence of a disallowed form. For example, if the required narration name is "Asher", a narration that uses "Asher" in one sentence and "Su Ming" in another still has a FAIL for the unauthorized "Su Ming" occurrence. The translation may correctly retain the canonical identity while narration follows its authorized name; attribute this finding to the NARRATION, not the TRANSLATION. Do not deterministically fail ai_contextual or manual localized naming choices: when the mapping permits contextual choice or notes require human interpretation, treat uncertain naming concerns as warn at most, unless there is independent evidence of a genuinely wrong entity. For consistent reconciliation, phrase a confirmed explicit rule violation as: The fault lies in the NARRATION: it uses "wrong form" instead of the required "authorized form" under the [rule] rule.

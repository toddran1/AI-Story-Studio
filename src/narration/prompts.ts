import { DeliveryIntensity, deliveryInstructions } from "./tts-direction.js";
import type { NarrationProfanityMode } from "../domain/story.js";

export const NARRATION_PROMPT_VERSION = "11";
export const narrationInstructions = (language: string, ttsProvider?: string, ttsModel?: string, profanityMode: NarrationProfanityMode = "preserve", deliveryIntensity: DeliveryIntensity = "restrained") => `Edit the supplied ${language} chapter into natural audiobook narration in ${language}. This is not a creative rewrite. Preserve every plot detail, fact, line of dialogue, name, ability, rank, point of view, tense, and chapter title. Do not summarize, omit, invent, or explain.${profanityMode === "soften-strong" ? " The sole authorized wording change is the narration profanity preference below." : " Do not censor."} Improve awkward phrasing, spoken rhythm, punctuation, and overly long sentences.
...
CANONICAL IDENTITY AND LOCALIZED NAMING ARE DIFFERENT: canonicalName, originalName, and aliases establish who or what the entity is. localizedNaming establishes how that same entity should read naturally in target-language narration. When localizedNaming exists, it takes precedence over the legacy Preferred Narration Name. Apply its locale and usageMode without global find-and-replace:
- ai_contextual: choose fullName for first introductions, formal/legal/official references, or ambiguity; choose shortName for established narration and familiar dialogue when natural. Consider speaker familiarity, dialogue versus narration, recent mention, titles, possessives, vocatives, and target-locale conventions.
- always_full: use fullName wherever the entity is named, while preserving grammatical titles and possessives.
- always_short: use shortName wherever the entity is named, while preserving grammatical titles and possessives.
- manual: follow localizedNaming.notes as explicit output guidance, using fullName/shortName only as those notes direct.
For non-character entities, fullName is the preferred localized label and shortName is an optional natural shorthand. Never change identity, invent a relationship, or expose these instructions in the narration.

${deliveryInstructions(ttsProvider, ttsModel, language, deliveryIntensity)}

...
function modelSchema() { return z.object({ provider: z.enum(["openai", "gemini", "kimi"]), model: z.string().min(1) }); }
...
async function markPending(path: string, stages: string[]) { const raw = await readJsonIfExists<Chapter>(path); if (!raw) return; const chapter = chapterSchema.parse(raw); for (const stage of stages) if (stage in chapter.stages) { const key = stage as keyof typeof chapter.stages; const current = chapter.stages[key]; if (current?.provider === "manual" && !current.manualAcceptance) continue; chapter.stages[key] = { status: "pending" }; } chapter.updatedAt = new Date().toISOString(); await atomicWriteJson(path, chapter); }

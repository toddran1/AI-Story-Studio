...
Enrichment caches suggestions and ordinary-English/no-pronunciation outcomes by canonical identity and source information; merely running enrichment, opening the desk, or processing chapters never activates a suggestion. Manual, custom, and locked records are protected through rebuilds and extraction regeneration. Clearing a configuration (**Use default TTS**) returns the entity to provider-default pronunciation with no placeholder record. Enrichment and uncached tests use configured providers and may incur charges; replaying an unchanged pronunciation test uses its saved preview.
...
The CLI reuses the web operations and canonical overlays. JSON output includes entity IDs and pronunciation records. Invalid arguments and missing entities/stories exit nonzero. API: `GET /api/stories/:slug/pronunciation` (entities plus cached suggestions), `GET/PUT /api/stories/:slug/pronunciation/:entityId`, `POST .../:entityId/accept-suggestion` or `.../:entityId/dismiss-suggestion`, and `POST .../:entityId/enrich` or `.../:entityId/test`; posting `{}` to the list endpoint generates suggestions for unconfigured entities. A PUT body is a pronunciation object or `null` to clear the override and return to default TTS. Jobs use the existing job-status endpoint.
...
Each Story Bible entity sheet has a collapsed **Visual evidence** section showing resolved values, chronology, and bounded source excerpts. Conflicting observations remain out of fallback artwork until an editor chooses a value, marks a later change, or dismisses an observation. Those decisions are stored in the manual canonical overlay and survive Story Bible rebuilds; they do not approve or rewrite a Visual Profile.

...
export class StudioOperations {
  ...
  async startQaRepair(slug: string, chapter: number, raw: unknown) {
    ...
    if (input.issueIndexes) snapshots = captureQaRepairFindingSnapshotsFromIssues(preflightPublicQa.issues, preflightState.findings, input.issueIndexes);
      ...
      snapshots = ids.map((id) => {
        ...
        return { id: finding.id, fingerprint: finding.fingerprint, ...captureQaRepairTextSnapshot(preflightTranslation, preflightNarration) };
      });
    }
    if (input.issueIndexes) {
      const textSnapshot = captureQaRepairTextSnapshot(preflightTranslation, preflightNarration);
      snapshots = snapshots.map((snapshot) => ({ ...snapshot, ...textSnapshot }));
    }
    ...
    return this.jobs.create("qaRepair", slug, async (control) => withStoryLock(this.root, slug, "selected QA repair", async () => {
      ...
      validateQaRepairTextSnapshot(snapshots[0]!, translationArtifact, narrationArtifact);
  ...
  private async mutateQaFindingState(slug: string, chapter: number, id: string, action: QaFindingTransition, options: { reason?: string; finalTextFingerprint?: string; qaStage?: Chapter["stages"]["qa"] } = {}) {
    const paths = storyPaths(this.root, slug, chapter);
    const [qaRaw, chapterRaw] = await Promise.all([readJsonIfExists(paths.qa), readJsonIfExists<Chapter>(paths.chapterMeta)]);
    if (!qaRaw || !chapterRaw) throw new Error(`Chapter ${chapter} does not have a QA result`);
    const metadata = chapterSchema.parse(chapterRaw);
      ...
      await persistQaStateWithMetadata(paths, state, metadata, (outputFingerprint, prior) => ({
        ...
        // After an AI text edit, retain the prior completed QA input fingerprint
        // so the saved review remains explicitly stale until a successful recheck.
  ...
  async startQaFindingFix(slug: string, chapter: number, id: string, raw?: unknown) {
    ...
    const selection: QaRepairFindingSnapshot = { id: preflightFinding.id, fingerprint: preflightFinding.fingerprint, ...captureQaRepairTextSnapshot(preflightTranslation, preflightNarration) };
    ...
    return this.jobs.create("qaRepair", slug, async (control) => withStoryLock(this.root, slug, "QA finding AI fix", async () => {
      ...
      validateQaRepairTextSnapshot(selection, translation, narration);
      const chapterBeforeRepair = chapterSchema.parse(await readJsonIfExists<Chapter>(paths.chapterMeta));
      ...
      await this.mutateQaFindingState(slug, chapter, id, "ai_fix", { finalTextFingerprint: fingerprint({ translation: repairedTranslation, narration: repairedNarration }), qaStage: chapterBeforeRepair.stages.qa });
      let recheck: Awaited<ReturnType<typeof recheckChapterQa>>;
      try {
  ...
  private async applyCanonicalEntityPatch(slug: string, id: string, raw: unknown) {
    ...
    try {
      entity = result.bible.canonicalEntities.find((item) => item.id === id); if (!entity) throw new Error("Canonical entity was not found after update");
      // Field-change-aware invalidation: downstream scans only run for the
      // change classes that can actually affect them. Notes/metadata-only
      // edits pay no chapter-wide scan at all.
      const namingChanged = narrationNamingChanged(before, entity);
      const pronunciationChanged = JSON.stringify(before.pronunciation ?? null) !== JSON.stringify(entity.pronunciation ?? null);
      const typeChanged = before.type !== entity.type;
      visualProfileReviewRequired = typeChanged && Boolean((await loadVisualProfiles(this.root, slug))[id]);
      invalidation = namingChanged
        ? await invalidateNarrationNamingChange(this.root, slug, before, entity)
        : { affectedChapters: [] as number[], manualNarrationChapters: [] as number[], exportCleanupWarnings: [] as string[] };
      const soundAffected = pronunciationChanged ? await invalidatePronunciationChange(this.root, slug, before, entity) : [];
      invalidation.affectedChapters = [...new Set([...invalidation.affectedChapters, ...soundAffected])];
      if ((raw as { pronunciation?: unknown }).pronunciation === null) await clearPronunciationAttempt(this.root, slug, id);
      if (typeChanged) await invalidateCanonicalIdentityChange(this.root, slug, [before, entity], `Canonical entity type changed for ${entity.canonicalName}`, Boolean(entity.preferredNarrationName || entity.localizedNaming || entity.aliasNarrationRules.length));
      if (visualProfileReviewRequired) await updateVisualProfile(this.root, slug, id, { status: "draft" });
    }
    catch (error) {

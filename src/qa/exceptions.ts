...
import { QaExceptionTooBroadError } from "./errors.js";
...
export async function addQaException(
  ...
  const prepared = prepareQaException(exceptions, input);
  if (!prepared.created) return prepared;
  const { exception, exceptions: next } = prepared;
  await atomicWriteJson(exceptionsPath(root, slug), { version: 1, exceptions: next });
  return prepared;
}

export function prepareQaException(
  exceptions: QaException[],
  input: { category: QaException["category"]; matchKind: QaException["matchKind"]; value: string; reason?: string },
): { exception: QaException; exceptions: QaException[]; created: boolean } {
  const value = input.value.trim();
  if (!value) throw new Error("QA exception value must not be empty");
  validateExceptionSpecificity(input.matchKind, value);
  ...
  return { exception, exceptions: [...exceptions, exception], created: true };
}
...
export function filterExceptedFindings<T extends FreshQaDetection>(detections: T[], exceptions: QaException[]): T[] {
  ...
  return detections.filter((detection) => !exceptions.some((exception) => {
    ...
    if (exception.matchKind === "entity") return !detection.entityMatchAmbiguous && (detection.entityIds?.includes(exception.value) ?? false);
    if (exception.matchKind === "rule") return detection.ruleKey === exception.value;
    return phraseMatches(`${detection.message}\n${detection.evidence}`, exception.value);
  }));
}

export function validateExceptionSpecificity(matchKind: QaException["matchKind"], value: string): void {
  if (matchKind === "entity" || matchKind === "rule") return;
  const normalized = normalizeQaText(value);
  const tokens = normalized.split(" ").filter(Boolean);
  const generic = new Set(["king", "man", "city", "name", "person", "thing", "good", "bad", "wrong", "right"]);
  if (normalized.length < 4 || tokens.length === 1 && generic.has(normalized)) {
    throw new QaExceptionTooBroadError("This exception is too broad. Use a more specific phrase, canonical entity ID, or stable QA rule key.");
  }
}

function phraseMatches(text: string, value: string): boolean {
  const haystack = ` ${normalizeQaText(text)} `;
  const needle = ` ${normalizeQaText(value)} `;

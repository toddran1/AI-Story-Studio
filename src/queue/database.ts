...
export async function assertQueueSchema(pool: DatabasePool) {
  let result;
  try { result = await pool.query<{ version: string }>("SELECT version FROM schema_migrations WHERE version = ANY($1::text[])", [["001_durable_production_queue", "002_queue_lease_fencing", "003_provider_usage"]]); }
  ...
  if (result.rowCount !== 3) throw new Error("Database migrations are incomplete. Run `npm run db:migrate`.");
}

...
- **Vendor isolation**: provider-specific code exists *only* in `src/llm/openai` (Responses API, JSON Schema structured output), `src/llm/gemini` (Interactions API), `src/llm/kimi` (Moonshot OpenAI-compatible Chat Completions), `src/artwork/openai-image.provider.ts` + `src/artwork/gemini-image.provider.ts` (image generation adapters over the provider-neutral contract in `src/artwork/provider.ts`; defaults/compatibility in `src/artwork/providers.ts`), and `src/tts/fish`. Everything else talks to generic LLM/TTS/image contracts. Remote transport/caching is in `src/source/web`; Fanqie specifics in `src/source/fanqie`.

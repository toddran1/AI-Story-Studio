...
describe("cost calculation",()=>{
  ...
  it("builds transparent historical estimates and preserves unknown stages",()=>{const plan:any={counts:{translation:{required:3,reusable:2},tts:{required:2,reusable:3}},stages:["translation","tts"]};const result=estimatePlanCost(plan,{dimensions:[{stage:"translation",requests:10,costUsd:2}]});expect(result).toMatchObject({classification:"historical",estimatedUsd:.6,unknownStages:["tts"]});expect(result.assumptions.join(" ")).toMatch(/Cached stages/);});
});

...
function StoryPage({ slug, navigate, onJob }: { slug: string; navigate: (path: string) => void; onJob: (job: Job) => void }) {
  ...
  const [chapters, setChapters] = useState<{ items: ChapterRow[]; page: number; pages: number; total: number }>(); const [range, setRange] = useState({ from: "1", to: "10", force: "", stage: "", mode: "selected", continueOnError: false }); const [error, setError] = useState(""); const [refreshResult, setRefreshResult] = useState<any>(); const [stagePreview, setStagePreview] = useState<any>();
  useEffect(() => { api<StoryDashboard>(`/stories/${slug}/dashboard`).then((value) => { setOverview(value); setRange({ from: value.counts.minChapter ? String(value.counts.minChapter) : "", to: value.counts.maxChapter ? String(value.counts.maxChapter) : "", force: "", stage: "", mode: "selected", continueOnError: false }); }).catch((e) => setError(message(e))); }, [slug]);
  useEffect(() => { api<any>(`/stories/${slug}/chapters?page=${page}&pageSize=${pageSize}&filter=${filter}&q=${encodeURIComponent(deferredQuery)}`).then(setChapters).catch((e) => setError(e.message)); }, [slug, page, pageSize, filter, deferredQuery]);
  const selectMatchingChapters = async () => { try { setError(""); const selected: number[] = []; let nextPage = 1; let pages = 1; do { const result = await api<{ items: ChapterRow[]; pages: number }>(`/stories/${slug}/chapters?page=${nextPage}&pageSize=100&filter=${filter}&q=${encodeURIComponent(deferredQuery)}`); selected.push(...result.items.map((item) => item.chapter)); pages = result.pages; nextPage++; } while (nextPage <= pages); setSelectedChapters((current) => [...new Set([...current, ...selected])]); } catch (value) { setError(message(value)); } };
  useEffect(() => { if (!range.stage || !Number(range.from) || !Number(range.to) || Number(range.to) < Number(range.from)) { setStagePreview(undefined); return; } const selected = Array.from({ length: Number(range.to) - Number(range.from) + 1 }, (_, index) => Number(range.from) + index); post<any>(`/stories/${slug}/stages/plan`, { chapters: selected, stage: range.stage, mode: range.mode, dryRun: true }).then(setStagePreview).catch(() => setStagePreview(undefined)); }, [slug, range.from, range.to, range.stage, range.mode]);
  const start = async () => { try { setError(""); const job = await post<Job>(`/stories/${slug}/jobs/batch`, { from: Number(range.from), to: Number(range.to), force: range.stage ? undefined : range.force || undefined, stage: range.stage || undefined, mode: range.mode, continueOnError: range.continueOnError }); onJob(job); } catch (e) { setError(message(e)); } };
  ...
  return <section className="page dashboard-page"><div className="dashboard-hero"><div><span className="eyebrow">Production dashboard</span><h2>{overview.story.title}</h2><p>{overview.story.author ?? "Author not listed"} · {overview.story.sourceLanguage} → {overview.story.outputLanguage} · {pretty(overview.story.source.type)} source</p><small>{overview.source ? `${overview.source.chapterCount} chapters imported ${new Date(overview.source.importedAt).toLocaleDateString()}` : "Source manifest unavailable"}</small></div><div className="quality-orbit"><strong>{overview.progress.processed}</strong><span>processed / {overview.counts.chapters}</span></div></div>
    ...
    <div className="processing-panel">
      <div className="processing-intro"><span className="eyebrow">Batch processing</span><p>{range.stage ? "Review the execution map, then run only the stage work you intend." : "Choose a chapter range for normal pipeline processing."}</p></div>
      <div className="processing-controls">
        <Field label="From"><input value={range.from} onChange={(e) => setRange({ ...range, from: e.target.value })} inputMode="numeric" /></Field>
        <Field label="To"><input value={range.to} onChange={(e) => setRange({ ...range, to: e.target.value })} inputMode="numeric" /></Field>
        <Field label="Manual stage"><select value={range.stage} onChange={(e) => setRange({ ...range, stage: e.target.value, force: e.target.value ? "" : range.force })}><option value="">Normal batch</option>{["translation", "narration", "qa", "storyBible", "continuity", "tts", "audioMastering", "alignment", "subtitles", "scenePlanning", "artwork", "video"].map((x) => <option key={x} value={x}>{pretty(x)}</option>)}</select></Field>
        {range.stage ? <Field label="Processing mode"><select value={range.mode} onChange={(e) => setRange({ ...range, mode: e.target.value })}><option value="selected">Selected stage</option><option value="through">Selected + all prior</option></select></Field> : <Field label="Force stage"><select value={range.force} onChange={(e) => setRange({ ...range, force: e.target.value })}><option value="">Reuse valid work</option>{["translation", "narration", "qa", "story-bible", "continuity", "tts", "audio", "all"].map((x) => <option key={x}>{x}</option>)}</select></Field>}
        <label className="batch-continue"><input type="checkbox" checked={range.continueOnError} onChange={(e) => setRange({ ...range, continueOnError: e.target.checked })} /><span aria-hidden="true" /><div><b>Continue past failures</b><small>Keep processing. Failed chapters go to Needs Review.</small></div></label>
        <button className="button primary" onClick={start}>Start processing</button>
      </div>
      {stagePreview && <StagePlanPreview preview={stagePreview} />}
    </div>
...
function StagePlanPreview({ preview }: { preview: any }) {
  const plans = preview.chapters ?? [];
  const displayedPlans = plans.slice(0, 3);
  const rebuildingCount = plans.filter((plan: any) => plan.missingStages?.length > 0).length;
  return <section className="stage-plan-preview" aria-live="polite">
    <header>
      <div><span className="eyebrow">Execution preview</span><b>{plans.length} chapter plan{plans.length === 1 ? "" : "s"}</b></div>
      <small>{rebuildingCount ? `${rebuildingCount} chapter${rebuildingCount === 1 ? "" : "s"} will rebuild missing prerequisites.` : "Required artifacts are available to reuse."}</small>
    </header>
    <div className="stage-plan-ledger">
      {displayedPlans.map((plan: any) => <article key={plan.chapter}>
        <strong>Ch. {String(plan.chapter).padStart(4, "0")}</strong>
        <div className="stage-plan-trail">{plan.runStages.map((stage: string, index: number) => <span key={stage} className={plan.missingStages?.includes(stage) ? "rebuild" : ""}>{pretty(stage)}{index < plan.runStages.length - 1 && <i aria-hidden="true">→</i>}</span>)}</div>
        <small className="stage-plan-reason">{plan.missingStages?.length ? `Rebuilds ${plan.missingStages.map(pretty).join(", ")}.` : plan.reusedStages?.length ? `Reuses ${plan.reusedStages.map((item: any) => `${pretty(item.stage)} ${item.state}`).join(", ")}.` : "Runs selected work."}</small>
      </article>)}
    </div>
    {plans.length > displayedPlans.length && <footer>+ {plans.length - displayedPlans.length} additional chapters will follow their own validated plan.</footer>}
  </section>;
}
  ...
  return <section className="page">
    <div className="section-heading">
      ...
      <div className="section-heading-actions">
        <button className="button" onClick={() => setResetModalOpen(true)}>Reset QA data</button>
      </div>
    </div>
...
function VideoPage({ slug, onJob }: { slug: string; onJob: (job: Job) => void }) {
  ...
  return <section className="page video-page"><div className="section-heading"><div><span className="eyebrow">Picture desk</span><h2>Chapter video editions</h2><p>Mastered narration becomes a restrained, readable screen edition.</p></div><div className="video-format-stamp">{data.settings.width}<i>×</i>{data.settings.height}<small>{data.settings.fps} FPS · H.264</small></div></div>
    ...
    {error && <ErrorBox text={error} />}<div className="video-ledger"><section><div className="ledger-head"><h3>Render queue</h3><span className="mono">{data.counts.videos} ready</span></div>{data.chapters.map((chapter) => <article key={chapter.chapter}><span className="chapter-index">{String(chapter.chapter).padStart(4, "0")}</span><div><b>{chapter.title ?? `Chapter ${chapter.chapter}`}</b><small>{chapter.durationSeconds ? `${formatDuration(chapter.durationSeconds)} audio` : "Master audio first"}</small></div><Stage value={chapter.subtitleStatus} /><Stage value={chapter.videoStatus} />{chapter.videoAvailable ? <>{chapter.videoStale && <Status status="warn" label="stale" />}<video controls preload="none" src={`/api/stories/${slug}/chapters/${chapter.chapter}/video`} /></> : <span className="render-empty">—</span>}</article>)}</section><aside><h3>Combined editions</h3>{data.exports.length ? data.exports.map((item) => <a href={item.downloadUrl} download key={item.fingerprint}><span>MP4</span><div><b>Chapters {item.from}–{item.to}</b><small>{formatDuration(item.durationSeconds)} · {new Date(item.createdAt).toLocaleString()}</small></div><strong>↓</strong></a>) : <Empty title="No video edition yet" text="Render a range, then build one continuous MP4." />}</aside></div>
  </section>;
}

function AudioPage({ slug, onJob }: { slug: string; onJob: (job: Job) => void }) {
  const [data, setData] = useState<AudioDashboard>(); const [range, setRange] = useState({ from: "", to: "", format: "m4b" as "mp3" | "m4b" }); const [page, setPage] = useState(1); const [pageSize, setPageSize] = useState(() => chapterPageSize(location.search)); const [error, setError] = useState(""); const watcher = useRef<(() => void) | undefined>(undefined);
  ...
  if (error && !data) return <LoadFailure error={error} />; if (!data) return <Loading />;
  const masters = paginateRows(data.chapters, page, pageSize);
  return <section className="page audio-page"><div className="section-heading"><div><h2>Audio mastering &amp; export</h2><p>Polished chapter masters and bookmarkable audiobook editions.</p></div><Status status={data.counts.mastered !== data.counts.total || !data.counts.total ? "pending" : data.counts.stale ? "warn" : "pass"} label={`${data.counts.mastered} / ${data.counts.total} available${data.counts.stale ? ` · ${data.counts.stale} stale` : ""}`} /></div>
    ...
    {error && <ErrorBox text={error} />}<div className="audio-layout"><section><div className="audio-master-heading"><h3>Chapter masters</h3><label className="chapter-page-size"><span>Chapters per page</span><select value={pageSize} aria-label="Chapter masters per page" onChange={(event) => { const next = Number(event.target.value); setPageSize(next); setPage(1); const url = new URL(location.href); url.searchParams.set("pageSize", String(next)); history.replaceState({}, "", `${url.pathname}${url.search}`); }}><option value="10">10</option><option value="25">25</option><option value="50">50</option><option value="100">100</option></select></label></div><div className="master-list">{masters.items.map((chapter) => <article key={chapter.chapter}><span className="mono">{String(chapter.chapter).padStart(4, "0")}</span><div><b>{chapter.title ?? `Chapter ${chapter.chapter}`}</b><small>{chapter.durationSeconds ? formatDuration(chapter.durationSeconds) : chapter.audioAvailable ? (chapter.audioStale ? "Retained master" : "Mastered") : "Awaiting master"}</small></div><div className="master-flags"><Stage value={chapter.status} />{chapter.audioStale && chapter.status !== "stale" && <Status status="warn" label="stale" />}</div>{chapter.audioAvailable && <div className="master-audio"><audio controls preload="none" src={`/api/stories/${slug}/chapters/${chapter.chapter}/audio`} /><a className="audio-download" href={`/api/stories/${slug}/chapters/${chapter.chapter}/audio`} download>Download MP3</a></div>}</article>)}</div>{masters.total > 0 && <div className="pagination"><button disabled={masters.page === 1} onClick={() => setPage(masters.page - 1)}>Previous</button><span className="mono">Page {masters.page} / {masters.pages} · {masters.total} chapters</span><button disabled={masters.page === masters.pages} onClick={() => setPage(masters.page + 1)}>Next</button></div>}</section><section><h3>Exports</h3><div className="export-list">{data.exports.length ? data.exports.map((item) => <a href={item.downloadUrl} key={item.fingerprint} download><span>{item.format.toUpperCase()}</span><div><b>Chapters {item.from}–{item.to}</b><small>{formatDuration(item.durationSeconds)} · {new Date(item.createdAt).toLocaleString()}</small></div><strong>↓</strong></a>) : <Empty title="No editions yet" text="Choose a range and build your first MP3 or M4B." />}</div></section></div>
    {error && <ErrorBox text={error} />}<div className="audio-layout"><section><div className="audio-master-heading"><h3>Chapter masters</h3><label className="chapter-page-size"><span>Chapters per page</span><select value={pageSize} aria-label="Chapter masters per page" onChange={(event) => { const next = Number(event.target.value); setPageSize(next); setPage(1); const url = new URL(location.href); url.searchParams.set("pageSize", String(next)); history.replaceState({}, "", `${url.pathname}${url.search}`); }}><option value="10">10</option><option value="25">25</option><option value="50">50</option><option value="100">100</option></select></label></div>{masters.total > 0 && <Pagination position="top" page={masters.page} pages={masters.pages} total={masters.total} itemLabel="chapters" onPrevious={() => setPage(masters.page - 1)} onNext={() => setPage(masters.page + 1)} />}<div className="master-list">{masters.items.map((chapter) => <article key={chapter.chapter}><span className="mono">{String(chapter.chapter).padStart(4, "0")}</span><div><b>{chapter.title ?? `Chapter ${chapter.chapter}`}</b><small>{chapter.durationSeconds ? formatDuration(chapter.durationSeconds) : chapter.audioAvailable ? (chapter.audioStale ? "Retained master" : "Mastered") : "Awaiting master"}</small></div><div className="master-flags"><Stage value={chapter.status} />{chapter.audioStale && chapter.status !== "stale" && <Status status="warn" label="stale" />}</div>{chapter.audioAvailable && <div className="master-audio"><audio controls preload="none" src={`/api/stories/${slug}/chapters/${chapter.chapter}/audio`} /><a className="audio-download" href={`/api/stories/${slug}/chapters/${chapter.chapter}/audio`} download>Download MP3</a></div>}</article>)}</div>{masters.total > 0 && <Pagination position="bottom" page={masters.page} pages={masters.pages} total={masters.total} itemLabel="chapters" onPrevious={() => setPage(masters.page - 1)} onNext={() => setPage(masters.page + 1)} />}</section><section><h3>Exports</h3><div className="export-list">{data.exports.length ? data.exports.map((item) => <a href={item.downloadUrl} key={item.fingerprint} download><span>{item.format.toUpperCase()}</span><div><b>Chapters {item.from}–{item.to}</b><small>{formatDuration(item.durationSeconds)} · {new Date(item.createdAt).toLocaleString()}</small></div><strong>↓</strong></a>) : <Empty title="No editions yet" text="Choose a range and build your first MP3 or M4B." />}</div></section></div>
  </section>;
}
...
function ProductionPage({ slug, activeJob, onJob, navigate }: { slug: string; activeJob?: Job; onJob: (job: Job) => void; navigate:(path:string)=>void }) {
...
export function QaDetail({ slug, chapter, onJob, onEditManually, onChanged, initialData }: { slug: string; chapter: number; onJob: (job: Job) => void; onEditManually: () => void; onChanged: () => void; initialData?: ChapterQaDetail }) {
  ...
  if (error && !data) {
    ...
    return <div className="qa-detail"><ErrorBox text={error} /></div>;
  }
  if (!data) return <Loading />;

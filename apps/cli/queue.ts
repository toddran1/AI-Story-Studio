...
const args=parse(process.argv.slice(2));const env=loadEnvironment();const root=resolveStudioRoot(env);if(!env.DATABASE_URL)throw new Error("DATABASE_URL is required for durable queue submissions");const pool=createDatabasePool(env.DATABASE_URL);try{await assertQueueSchema(pool);const repository=new PostgresQueueRepository(pool,env.QUEUE_EVENT_RETENTION_DAYS);const service=new ProductionQueueService(root,env,repository);const job=await service.submit(args.story,{from:args.from,to:args.to,profile:args.profile,outputs:args.outputs});process.stdout.write(`Queued ${job.id}: ${job.story} Chapters ${job.from}–${job.to} (${job.totalItems} chapters require work)\n`);}finally{await pool.end();}

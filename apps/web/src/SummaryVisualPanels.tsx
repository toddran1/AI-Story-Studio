...
import { api, del, getArtDirection, post, put, type ArtworkVersion, type Job, type StoryArtDirection, type StoryConfig, type StorySummary } from "./api.js";
...
function recordValue(value: unknown): Record<string, any> | undefined { return value && typeof value === "object" && !Array.isArray(value) ? value as Record<string, any> : undefined; }
export function SelectedArtworkProvenance({ version, approvedVersionNumber }: { version: ArtworkVersion; approvedVersionNumber?: number }) {
  const provenance = recordValue(version.provenance);
  const direction = recordValue(provenance?.artDirection);
  const continuity = recordValue(provenance?.continuityReference);
  const visualCanon = Array.isArray(provenance?.visualCanon) ? provenance.visualCanon.map(recordValue).filter((value): value is Record<string, any> => Boolean(value)) : undefined;
  return <VisualGroundingPanel label={`Grounding recorded for selected v${version.versionNumber}`} recorded={Boolean(provenance)}>
    <span>Selected version · v{version.versionNumber}</span>
    <span>Approved production version · {approvedVersionNumber ? `v${approvedVersionNumber}` : "none"}</span>
    {direction ? <span>Art Direction · {direction.source ?? "recorded"}{direction.presetName ? ` · ${direction.presetName}` : ""}{direction.missingPresetId ? ` · missing preset ${direction.missingPresetId}; fallback used` : ""}</span> : <span>Art Direction fingerprint · {version.artDirectionFingerprint || "not recorded"}</span>}
    {(version.resolvedVisualProfileReferences ?? []).length > 0 && <><small>Resolved Visual Profile references</small>{version.resolvedVisualProfileReferences!.map((reference) => <span key={`${reference.entityId}:${reference.referenceId ?? ""}`}>{reference.name ?? reference.entityId}{reference.referenceId ? ` · reference ${reference.referenceId}` : reference.role ? ` · ${reference.role}` : ""}</span>)}</>}
    {visualCanon ? visualCanon.length ? visualCanon.map((item, index) => <span key={item.entityId ?? item.name ?? index}>{item.name ?? item.entityId ?? "Entity"} — {item.source ?? "source not recorded"}{item.primaryReference ? " · primary reference" : item.reference ? " · approved reference" : ""}</span>) : <span>No resolved canonical entities were recorded.</span> : <span>Visual grounding details not recorded.</span>}
    {provenance?.referencesUsed !== undefined && <span>Reference mode · {String(provenance.referencesUsed)}{typeof provenance.referenceImageCount === "number" ? ` · ${provenance.referenceImageCount} image${provenance.referenceImageCount === 1 ? "" : "s"}` : ""}</span>}
    {continuity && <span>Previous-scene reference · {continuity.used ? `${continuity.sourceSceneId ?? "previous scene"}${continuity.versionNumber ? ` · v${continuity.versionNumber}` : ""} used` : continuity.reason ?? "not used"}</span>}
...
export function SummaryArtworkPanel(props: SummaryVisualProps) {
  ...
  return <section className="summary-media-editor"><header><span className="eyebrow">Canonical visual continuity</span><h3>Scene artwork</h3><p>Uses this book’s image provider, style, and canonical visual references. Approved work is protected unless you explicitly regenerate it.</p></header>
    ...
    <div className="summary-artwork-grid">{visibleScenes.map((scene) => { const versions = scene.artwork.versions ?? []; const chosen = versions.find((item) => item.id === selectedVersionByScene[scene.id]) ?? versions.find((item) => item.id === scene.artwork.approvedVersionId) ?? versions.at(-1); const url = chosen ? `/api${props.base}/artwork/${scene.id}/versions/${chosen.id}` : `/api${props.base}/artwork/${scene.id}?v=${scene.artwork.imageFingerprint ?? ""}`; const state = grounding.find((item) => item.sceneId === scene.id); return <article className="summary-scene-card" key={scene.id}><header><label><input type="checkbox" checked={selected.includes(scene.id)} onChange={(event) => setSelected(event.target.checked ? [...selected, scene.id] : selected.filter((id) => id !== scene.id))} /> {scene.id}</label><span>{state?.status === "stale" ? "Scene changed — artwork is stale" : state?.status === "missing" ? "Artwork missing" : state?.status === "current" ? "Artwork current" : scene.artwork.status} · {scene.artwork.review}</span></header>
      ...
      {chosen && <div className="summary-meta"><span>{new Date(chosen.createdAt).toLocaleString()}</span><span>{chosen.provider} · {chosen.model}</span>{chosen.original && <span>Original {chosen.original.width}×{chosen.original.height}</span>}{chosen.upscale?.finalDimensions && <span>Production {chosen.upscale.finalDimensions.width}×{chosen.upscale.finalDimensions.height}</span>}<span>Upscale: {chosen.upscale?.status ?? "not applied"}</span><span>{scene.artwork.approvedVersionId === chosen.id ? "Approved version" : chosen.id === versions.at(-1)?.id ? "Latest version" : "Historical version"}</span></div>}
      ...
      {chosen && <SelectedArtworkProvenance version={chosen} approvedVersionNumber={versions.find((version) => version.id === scene.artwork.approvedVersionId)?.versionNumber} />}

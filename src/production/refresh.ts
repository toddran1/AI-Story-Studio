...
export async function refreshProductionRange(options: { root: string; story: Story; from: number; to: number; registry: SourceProviderRegistry }) {
  ...
  const { provider } = await options.registry.resolve(manifest.origin.url, manifest.type); const directory = await options.registry.inspect(provider, manifest.origin.url, { refresh: true }); const comparison = compareRemoteDirectory(manifest, directory);
  ...
  const inspection = await options.registry.inspect(provider, manifest.origin.url, { chapters: requested.map((item) => item.chapter) }); const result = await importSource(options.root, options.story.slug, inspection, async () => { await atomicWriteJson(paths.storyConfig, options.story); await atomicWriteJson(paths.pipelineConfig, options.story.pipeline); });

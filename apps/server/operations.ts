  ...
  startAudio(slug: string, raw: unknown) {
    ...
    return this.jobs.create("audio", slug, async (control) => withStoryLock(this.root, slug, "web audio mastering", async () => {
      ...
      for (let index = 0; index < selected.length; index++) {
        if (shutdown.isRequested) return { status: "paused", mastered, reused, total: selected.length, warnings };
      ...
      return { status: "completed", mastered, reused, total: selected.length, warnings };

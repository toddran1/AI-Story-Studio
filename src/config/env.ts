...
const envSchema = z.object({
  ...
  ALIGNMENT_MODEL: optionalSecret,
  ALIGNMENT_DEVICE: z.enum(["auto", "cpu", "gpu"]).default("auto"),
  ALIGNMENT_MIN_MATCH_PERCENT: z.coerce.number().min(0).max(100).default(85),
  ALIGNMENT_MIN_CONFIDENCE: z.coerce.number().min(0).max(1).default(.45),
  ALIGNMENT_MAX_GAP_SECONDS: z.coerce.number().min(1).max(120).default(15),

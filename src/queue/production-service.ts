...
export class ProductionQueueService implements QueueExecutor {
  private readonly runtime; private readonly registry; private readonly video = new FfmpegVideoProcessor(); private readonly videoExport = new FfmpegVideoExportProcessor(); private readonly audiobook = new FfmpegAudiobookProcessor();
  constructor(private readonly root:string,private readonly env:Environment,readonly repository:PostgresQueueRepository){this.runtime=createPipelineRuntime(env);this.registry=new SourceProviderRegistry(undefined,createWebHttpClient(root,env));}

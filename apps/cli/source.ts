...
async function main() {
  ...
  const root = process.cwd(); const result = await importSource(root, args.story!, inspection);
  const paths = storyPaths(root, args.story!, inspection.chapters[0]?.ref.chapter ?? 1); const existed = await exists(paths.storyConfig);
  let story = existed ? await loadStory(paths.storyConfig) : defaultStory(args.story!, loadEnvironment());
  ...
  process.stdout.write(`${formatImport(args.story!, result)}\n`);
}
...
function parseArgs(values: string[]): Args {
  ...
  for (let index = 0; index < values.length; index++) {
    ...
    const value = values[++index]; if (!value) usage(`Missing value for ${key}`);
...
function usage(message: string): never { throw new Error(`${message}\nUsage: story:inspect -- --source <path> [--type text|epub|docx|manual|original] [--split-chapters] [--chapter N] [--allow-gaps]`); }

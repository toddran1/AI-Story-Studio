...
export function createApiHandler(operations: StudioOperations) {
  return async function handle(request: IncomingMessage, response: ServerResponse): Promise<boolean> {
    ...
    try {
      ...
      const scenesIndexRowMatch = /^\/api\/stories\/([a-z0-9-]+)\/scenes\/index\/(\d+)$/.exec(url.pathname);
      if (scenesIndexRowMatch && request.method === "GET") return send(response, 200, await getScenesIndexRow(operations.root, scenesIndexRowMatch[1]!, chapterParam(scenesIndexRowMatch[2]!)));
      ...
      const bibleReviewSummaryMatch = /^\/api\/stories\/([a-z0-9-]+)\/story-bible\/review\/summary$/.exec(url.pathname);
      if (bibleReviewSummaryMatch && request.method === "GET") return send(response, 200, await getStoryBibleReviewSummary(operations.root, bibleReviewSummaryMatch[1]!));
      ...
      if (bibleRestoreMatch && request.method === "POST") return send(response, 200, await operations.restoreCanonicalEntity(bibleRestoreMatch[1]!, bibleRestoreMatch[2]!));

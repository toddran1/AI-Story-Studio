...
export async function listStories(root: string, warnings: string[] = []) {
  ...
  const cards = await mapLimit(directories.filter((slug) => slugSchema.safeParse(slug).success), 4, async (slug) => {
    ...
    try {
      ...
      const chapters = await getChapterStatusReadModel(root, slug); const processed = chapters.filter((item) => item.audioAvailable || item.audioMastering === "complete");
...
export const settingsUpdateSchema = z.object({
  ...
  tts: z.object({ provider: ttsProviderNameSchema.optional(), model: z.string().trim().min(1).optional(), referenceId: z.string().trim().optional(), secondaryReferenceId: z.string().trim().optional(),
    voiceMode: z.enum(["narrator-only", "same-voice-dialogue", "narrator-dialogue"]).optional(), deliveryIntensity: z.enum(["none", "restrained", "expressive"]).optional(), qualityGuard: z.boolean().optional(), qualityMode: z.enum(["off", "verify", "auto_repair"]).optional(), providerQualityGuard: z.boolean().optional(), speed: z.number().min(0.5).max(2),
...
export async function updateStorySettings(root: string, slug: string, input: unknown): Promise<Story> {
  ...
  return withStoryLock(root, slug, "web settings update", async () => {
    ...
    const story = storySchema.parse({ ...current, title: update.title, author: update.author || undefined, description: update.description, tags: update.tags, notes: update.notes, sourceLanguage: update.sourceLanguage, outputLanguage: update.outputLanguage,
      ...
      pipeline: {
        ...
        tts: {
          ...
          qualityMode: update.tts.qualityMode ?? current.pipeline.tts.qualityMode,
    ...
    await invalidateStoryForConfigChange(root, slug, current, story); await atomicWriteJson(paths.storyConfig, story); await atomicWriteJson(paths.pipelineConfig, story.pipeline); await invalidateChapterStatusDerivedReads(root, slug); return story;
  });
}

const CHAPTER_STATUS_CACHE = "chapter_status";
export async function invalidateChapterStatusDerivedReads(root: string, slug: string) {
  const startedAt = Date.now();
  invalidateStoryReadCache(root, slug, CHAPTER_STATUS_CACHE);
  await bumpChapterStatusReadRevision(root, slug);
...
async function getChapterStatusProjection(root: string, slug: string): Promise<ChapterStatusProjection> {
  ...
  const { value, cacheHit } = await cachedStoryRead(CHAPTER_STATUS_CACHE, root, slug,
    () => getChapterStatusReadRevision(root, slug), async () => {

...
import { FileCookieStore } from "./cookie-store.js";
...
export function createWebHttpClient(root: string, env: Environment) {
  return new WebHttpClient({
    ...
    maxResponseBytes: env.WEB_MAX_RESPONSE_BYTES, maxRetries: env.WEB_MAX_RETRIES, maintainCookies: true, solveBrowserChallenge: true,
    allowedHosts: [
    ...
    cookieStore: new FileCookieStore(resolve(root, env.WEB_CACHE_DIR ?? "cache/web", "../cookies")),
  });
}

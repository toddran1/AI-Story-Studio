...
async function main() { const args = parse(process.argv.slice(2)); loadEnvironment(); await withStoryLock(process.cwd(), args.story, "combined video export", async () => { const story = await loadStory(storyPaths(process.cwd(), args.story, 1).storyConfig); const selected = selectChapterRange((await loadImportedChapters(process.cwd(), args.story)).chapters, args.from, args.to); const renderer = new FfmpegVideoProcessor(); for (let index = 0; index < selected.length; index++) { const chapter = selected[index]!.chapter; if (story.video.subtitleMode !== "none") await generateStoredSubtitles({ root: process.cwd(), story, chapter }); const result = await renderStoredChapterVideo({ root: process.cwd(), story, chapter, processor: renderer }); process.stdout.write(`[render ${index + 1}/${selected.length}] Chapter ${chapter}: ${result.reused ? "reused" : "rendered"}\n`); } const from = selected[0]!.chapter; const to = selected.at(-1)!.chapter; const result = await assembleVideoExport({ root: process.cwd(), story, from, to, processor: new FfmpegVideoExportProcessor(), force: args.force }); process.stdout.write(`${result.reused ? "Reused" : "Built"} ${result.manifest.output}\n`); }); }

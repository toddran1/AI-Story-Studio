...
import { buildStoryBackup, cleanupStory, createBlankStory, deleteStory, duplicateStory, getStorageUsage, loadGlobalSettings, readActivity, recordActivity, restoreStoryBackup, saveCover, saveGlobalSettings, updateStoryMetadata } from "../src/studio/projects.js";
...
describe("story project management", () => {
  ...
  it("duplicates settings alone or the full project and always chooses a unique slug", async () => {
    const directory = await root(); await createBlankStory(directory, env, metadata); const sourcePaths = storyPaths(directory, metadata.slug, 1); const sourceFile = join(sourcePaths.source, "chapters", "0001.txt"); await writeFile(sourceFile, "chapter", { encoding: "utf8", flag: "w" }).catch(async () => { const { atomicWrite } = await import("../src/storage/atomic-write.js"); await atomicWrite(sourceFile, "chapter"); }); await mkdir(join(sourcePaths.story, "exports"), { recursive: true }); await writeFile(join(sourcePaths.story, "exports", "old.json"), "{}"); await mkdir(join(sourcePaths.story, "production-runs"), { recursive: true }); await writeFile(join(sourcePaths.story, "production-runs", "latest.json"), "{}");
    ...
    expect(await exists(join(storyPaths(directory, full.slug, 1).story, "exports"))).toBe(false); expect(await exists(join(storyPaths(directory, full.slug, 1).story, "production-runs"))).toBe(false);
  });
  ...
  it("streams full backups containing media larger than the ingestion ZIP limit", async () => {
    const directory = await root(); await createBlankStory(directory, env, metadata); const video = storyPaths(directory, metadata.slug, 1).video; await mkdir(join(video, ".."), { recursive: true }); await writeFile(video, "media"); await truncate(video, 26 * 1024 * 1024);
    const backup = await buildStoryBackup(directory, metadata.slug, true); expect(backup.bytes).toBeGreaterThan(0); expect(await exists(join(directory, ".ai-story-studio", "backups", `${backup.id}.zip`))).toBe(true);
  });
  ...
  it("invalidates video only when a cover changes", async () => {
    ...
    await saveCover(directory, metadata.slug, "cover.png", png); const chapter = chapterSchema.parse(JSON.parse(await readFile(paths.chapterMeta, "utf8"))); expect(chapter.stages.video.status).toBe("pending"); expect(chapter.stages.tts.status).toBe("complete"); expect(chapter.stages.audioMastering.status).toBe("complete"); expect(chapter.stages.artwork.status).toBe("complete");
    await expect(saveCover(directory, metadata.slug, "broken.png", png.subarray(0, 12))).rejects.toThrow("structurally valid");
  });

  it("invalidates derived stages when project languages change", async () => {
    const directory = await root(); await createBlankStory(directory, env, metadata); const paths = storyPaths(directory, metadata.slug, 1); const now = new Date().toISOString(); const complete = { status: "complete" as const, fingerprint: "in", outputFingerprint: "out" };
    await atomicWriteJson(paths.chapterMeta, chapterSchema.parse({ chapter: 1, sourceLanguage: "zh-CN", outputLanguage: "en-US", counts: { originalCharacters: 10, englishWords: 2, narrationWords: 2 }, createdAt: now, updatedAt: now, stages: { ingestion: complete, translation: complete, narration: complete, qa: complete, storyBible: complete, tts: complete, audioMastering: complete, subtitles: complete, scenePlanning: complete, artwork: complete, video: complete } }));
    const exportManifest = join(paths.story, "exports", "old.m4b.json"); await atomicWriteJson(exportManifest, { output: "/old/path" });
    await updateStoryMetadata(directory, metadata.slug, { ...metadata, outputLanguage: "fr-FR" }); const chapter = chapterSchema.parse(JSON.parse(await readFile(paths.chapterMeta, "utf8")));
    expect(chapter.stages.ingestion.status).toBe("complete"); expect(chapter.stages.translation.status).toBe("pending"); expect(chapter.stages.audioMastering.status).toBe("pending"); expect(chapter.stages.video.status).toBe("pending"); expect(await exists(exportManifest)).toBe(false);
  });

  it("serializes concurrent activity updates", async () => {
    const directory = await root(); await createBlankStory(directory, env, metadata); await Promise.all(Array.from({ length: 25 }, (_, index) => recordActivity(directory, metadata.slug, "test", `Event ${index}`)));
    const activity = await readActivity(directory, metadata.slug, 100); expect(activity.filter((item) => item.type === "test")).toHaveLength(25);
  });
});

...
export async function planStoredScenes(options: { root: string; story: Story; chapter: number; provider: LLMProvider; force?: boolean }) {
  ...
  try {
    const result = await planScenes(options.provider, config, { chapter: options.chapter, title: chapter.translatedTitle ?? chapter.originalTitle, narration, durationSeconds: chapter.audio.durationSeconds, bible, settings: options.story.scenes });
    const scenes = normalizeSceneTiming(result.value.scenes.map((scene) => ({ ...scene, artwork: undefined as never })), chapter.audio.durationSeconds, options.story.scenes);
    ...
    validateSceneCoverage(scenes, chapter.audio.durationSeconds); const now = new Date().toISOString(); const manifest = sceneManifestSchema.parse({ version: 1, chapter: options.chapter, durationSeconds: chapter.audio.durationSeconds, planningFingerprint: inputFingerprint, planner: { provider: config.provider, model: config.model, promptVersion: SCENE_PLANNER_PROMPT_VERSION }, manualRevision: 0, manuallyEdited: false, createdAt: cached?.success ? cached.data.createdAt : now, updatedAt: now, scenes });
...
export async function updateStoredSceneManifest(options: { root: string; story: Story; chapter: number; scenes: unknown }) {
  ...
  validateSceneCoverage(scenes, manifest.durationSeconds); const updated = sceneManifestSchema.parse({ ...manifest, scenes, manuallyEdited: true, manualRevision: manifest.manualRevision + 1, updatedAt: new Date().toISOString() }); await atomicWriteJson(paths.scenesManifest, updated); await invalidateAfterSceneEdit(paths.chapterMeta, updated); return updated;
}

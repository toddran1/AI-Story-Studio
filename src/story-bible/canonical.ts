...
import { z } from "zod";
import { CanonicalEntity, StoryBible, canonicalEntitySchema, localizedNamingSchema, storyBibleSchema } from "../domain/story-bible.js";
...
const overrideSchema = z.object({ canonicalName: z.string().trim().min(1).max(300).optional(), aliases: z.array(z.string().trim().min(1).max(300)).max(100).optional(), canonicalNameLocked: z.boolean().optional(), notes: z.string().max(10_000).optional(), status: z.string().max(500).optional(), preferredNarrationName: z.string().trim().min(1).max(300).nullable().optional(), aliasNarrationRules: canonicalEntitySchema.shape.aliasNarrationRules.optional(), localizedNaming: localizedNamingSchema.nullable().optional(), updatedAt: z.string() });
...
export async function applyCanonicalOverlay(root: string, slug: string, input: StoryBible) {
  ...
  for (const entity of bible.canonicalEntities) { const value = overlay.overrides[entity.id]; if (!value) continue; if (value.canonicalName) { if (normalizeEntityName(value.canonicalName) !== normalizeEntityName(entity.canonicalName)) entity.aliases = unique([entity.canonicalName, ...entity.aliases]); entity.canonicalName = value.canonicalName; } if (value.aliases) entity.aliases = unique(value.aliases.filter((name) => normalizeEntityName(name) !== normalizeEntityName(entity.canonicalName))); if (value.canonicalNameLocked !== undefined) entity.canonicalNameLocked = value.canonicalNameLocked; if (value.notes !== undefined) entity.notes = value.notes; if (value.status !== undefined) entity.status = value.status; if (value.preferredNarrationName !== undefined) entity.preferredNarrationName = value.preferredNarrationName ?? undefined; if (value.localizedNaming !== undefined) entity.localizedNaming = value.localizedNaming ?? undefined; if (value.aliasNarrationRules) { const aliases = new Set(entity.aliases.map(normalizeEntityName)); entity.aliasNarrationRules = uniqueRules(value.aliasNarrationRules.filter((rule) => aliases.has(normalizeEntityName(rule.alias)))); } entity.origin = "manual"; }
  ...
  for (const [sourceId, targetId] of remap) {
    ...
    target.aliases = unique([...target.aliases, source.canonicalName, ...source.aliases]); target.description = mergeText(target.description, source.description); target.notes = mergeText(target.notes, source.notes); if (target.status === "unknown" && source.status !== "unknown") target.status = source.status; target.canonicalNameLocked ||= source.canonicalNameLocked; target.preferredNarrationName ??= source.preferredNarrationName; target.localizedNaming ??= source.localizedNaming; target.aliasNarrationRules = uniqueRules([...source.aliasNarrationRules, ...target.aliasNarrationRules]); target.firstAppearance = Math.min(target.firstAppearance, source.firstAppearance); target.lastKnownAppearance = Math.max(target.lastKnownAppearance, source.lastKnownAppearance); target.provenance = uniqueObjects([...target.provenance, ...source.provenance]); target.mergedFromIds = unique([...target.mergedFromIds, source.id, ...source.mergedFromIds]); target.origin = "manual";
  }
...
export async function updateCanonicalEntity(root: string, slug: string, base: StoryBible, id: string, patch: unknown) { const input = overrideSchema.omit({ updatedAt: true }).partial().strict().parse(patch); const entity = base.canonicalEntities.find((item) => item.id === id); if (!entity) throw new Error("Canonical entity was not found"); if (input.status !== undefined) { const status = input.status; input.status = normalizeEntityStatus(entity.type, status); if (!input.status || statusKey(status) === "custom") throw new Error("Custom status requires a non-empty story-specific value"); } const paths = storyPaths(root, slug, 1); const overlay = canonicalOverlaySchema.parse((await readJsonIfExists(paths.bibleCanonicalManual)) ?? { version: 1, overrides: {}, merges: [] }); overlay.overrides[id] = { ...overlay.overrides[id], ...input, updatedAt: new Date().toISOString() }; await atomicWriteJson(paths.bibleCanonicalManual, overlay); return applyCanonicalOverlay(root, slug, base); }

...
export function artworkPrompt(scene: Scene, refs: Array<{ name: string; description: string }>, style: string) { return [`STORY-WIDE ART DIRECTION: ${style}`, `SCENE: ${scene.visualPrompt}`, `SUMMARY: ${scene.summary}`, scene.location ? `LOCATION: ${scene.location}` : "", scene.characters.length ? `CHARACTERS: ${scene.characters.join(", ")}` : "", ...refs.map((ref) => `CANONICAL VISUAL REFERENCE — ${ref.name}: ${ref.description}`), "Create one polished still illustration. 16:9-safe composition. No text, captions, speech bubbles, logos, or watermarks."].filter(Boolean).join("\n"); }

...
async function main() { const args = parse(process.argv.slice(2)); loadEnvironment(); await withStoryLock(process.cwd(), args.story, "subtitle generation", async () => { const story = await loadStory(storyPaths(process.cwd(), args.story, 1).storyConfig); const imported = await loadImportedChapters(process.cwd(), args.story); const selected = selectChapterRange(imported.chapters, args.from, args.to); for (let index = 0; index < selected.length; index++) { const chapter = selected[index]!.chapter; const result = await generateStoredSubtitles({ root: process.cwd(), story, chapter, force: args.force }); process.stdout.write(`[${index + 1}/${selected.length}] Chapter ${chapter}: ${result.reused ? "reused" : "generated"}\n`); } }); }
